	...
	*new {| frames = 512, deckNum = 4 |
	...
	setEQBufs {
		decks.keysValuesDo{|key, deck|
			var instrumentName = (\AutomataRMX_binPassFilter_++key).asSymbol;
...
ShapeGenerator {
	var <frames, <shapes, <shape, <shapeTags, <curveTags;
...
MorphSequencers {
	...
	*new { | frames = 512, decks, analyzer, irBuffers|
		^super.new.init(frames, decks, analyzer, irBuffers)
	}

	init {|frames_, decks_, analyzer_, irBuffers_|
		frames = frames_;
		decks = decks_;
		analyzer = analyzer_;
		irBuffers = irBuffers_;
		server = Server.default;
		shapeGen = ShapeGenerator(frames);
	...
	fadeTo { | deckID, fadeTime , newBuffer |
		...
		//set played callback
		this.set_played(deckID, (fadeTime*2).round);

		Ndef(instruments[deckID]).xset(\inBuffer, newBuffer, \amp, 0.25); // Scaled to 1/4
	}
			...
			if(currentBuf > 0){
				{
				   time.wait;
					...
					analyzer[id].freeBuffer(currentBuf,id);
				}.fork;
			};
			};
	}
