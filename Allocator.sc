	...
	*new{|server,masterpath,trackpath,division,dynamic_allocation=false,maxBuffersAtTime,ext_callback|
	...
	init{
		if(File.exists(masterpath).not) {
			...
			outpath = trackpath;//(masterpath ++ "Tracks/");
	...
	load_splitted{
		...
		totalBuffers.do{|i|
			i.postln;
			files = SoundFile.collect(outpath++i++"/"++ "*");
			if((files != nil) && (files.size > 0)){
	...
	test_buffer{|bufnum|
		...
		if(bufnum != nil){
			d1 = Ndef(\AutomataRMX_binPassFilter_0).get(\inBuffer);
			d2 = Ndef(\AutomataRMX_binPassFilter_1).get(\inBuffer);
			d3 = Ndef(\AutomataRMX_binPassFilter_2).get(\inBuffer);
			d4 = Ndef(\AutomataRMX_binPassFilter_3).get(\inBuffer);
	...
	loadNext_dynamic{
		var files,rand,buf;
			...
			files = SoundFile.collect(outpath++current++"/"++ "*");
...
Splitter{
	...
	*new{|pdcpath,division="1.5",outpath,ext_callback|
	...
	run{|program, params|
		var cmd,sep = "\"";
		cmd = pdcpath++" "++program++" "++outpath++" "++outpath++" "++params;
		cmd.postln;
		cmd.unixCmd({|res,pid|
			"PDC ended.".postln;
