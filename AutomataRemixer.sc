...
// [http://www.darienbrito.com , http://www.typedef.altervista.org]
//
...
AutomataRemixer {
	...
	*create {
		...
		p = path;
		{
			...
			m = MorphSequencers(frames, d.decks, a, ir.buffers);
			//Decks masks
			t_seqs = decks.collect{|i| TendencyMask(100,10,20,40,60) };
			t_seqFades = decks.collect{|i| TendencyMask(100,4,6,7,9) };
			trackSequencers = decks.collect{|i| m.getTracksSequencer(i, t_seqs[i], t_seqFades[i])};
			// Parameters and Reverbs masks
			t_pmorphs = decks.collect{|i| TendencyMask(100,10,12,14,18) };
			...
			// Spectral EQ morpher
			t_smorph_time = TendencyMask.new(100, 6, 10, 12, 20);
			t_smorph_fade = TendencyMask.new(100, 8,15, 20, 25);
			shapeMorpher = m.getShapeMorpher(t_smorph_time, t_smorph_fade);
			// Macro Controls
			this.loadProfiles(path);
			playerMacros = AutomataMacros("PlayRate-Stretching Macro-Control",
			...
			revMacros = KernelAutomataMacros("Reverb Macro-Control",
			...
			decks.do{|i| Ndef((\AutomataRMX_binPassFilter_++i).asSymbol).set(\kernel, ir.buffers[4], \wet, 0.0)};
			mixer = Mixer();
			load = true;
		}.fork;
		this.message("ready!  ");
	...
	*makeGUI {
		...
		.action_({ |btn|
			if(load){
				if(btn.value > 0){
					this.play;
					masterWindow.setInnerExtent(550,800);
					...
					this.stop;
					masterWindow.setInnerExtent(550,75);
		...
		.action_({ |btn|
			if(btn.value > 0){
				var posX = 552, posY = 0;
				var posY2 = 0;
				masterWindow.setInnerExtent(550 + (293*2), 800);
				m.weightedShapes.makeInterface(posX, posY, masterWindow);
				m.weightedCurves.makeInterface(posX, 15*20, masterWindow);
				playerMacros.makeInterface(posX, 30*20, masterWindow);
				revMacros.makeInterface(posX + styler.ezSizeW + 10, 0, masterWindow);
				masksTags.do{|tag, i|
					timeMacroControllers[tag].makeInterface(posX + styler.ezSizeW + 10, (i+1) * 75 + 72, masterWindow);
				};
				Server.default.fancyMeter(masterWin: masterWindow, posX: 550 + (293), posY: 600);
			} {
				masterWindow.setInnerExtent(550, 800);
...
AutomataMacros {
	...
	*new{ | title, objs, profiles, morphers, wetControl = false |
		^super.new.init( title, objs, profiles, morphers, wetControl )
	}

	init { |title_, objs_, profiles_, morphers_, wetControl_ |
		title = title_;
		objs = objs_;
		profiles = profiles_;
...
KernelAutomataMacros : AutomataMacros {
	makeInterface { |posX = 0, posY = 0, masterWin|
		var styler = GUIStyler(masterWin);
		var numSliders = 2;
		var winWidth =  styler.ezSizeW + (styler.gadgetWidth/2);
