////////////////////////////////////////////////////////////////////////////////////////////////////
//
// BinPass2, a bin magnitude filter (FFT processing).
// Same as BinPass1 but with Ndefs. More expensive than
// BinPass because requires constant runnning of effects.
//
...
BinPass2 {
	// Private
	var sampleRate, presetCounter, dataTags, params, initVals, hertzRatio;
	...
	var <dataSet, <customDataSet, <eqBuffer, <instrument, <reverb, <interpolationData, <id;
	var <morpher, <bands, <skipjack, <customTags, <customEQfiles, <effects, <reverbParams;
	var <instrumentName, <reverbName, <isPlaying, <inspectorWindow;
	// Setters/Getters
	var <>masterPath, <>savedPresets, <>dataPaths, <>bufferPaths;
	var <>morphTime, <>stepsPerSec, <>curve, <>currentMorphType;
	var <>multi, <>debug, server;
	//var <masterWin;

	*new { |frames = 512, id = 0|
		^super.new.init(frames, id);
	}
	...
	init { |frames_, id_|
		...
		if(server.serverRunning) {
			...
			totalBins = frames / 2;
			eqBuffer = Buffer.alloc(server, frames, 1).zero;
			...
			morpher = Morpher(morphTime, stepsPerSec, curve, id);
			interpolateCondition = false;
			bands = this.getBands;
			instrumentName = (\AutomataRMX_binPassFilter_++id).asSymbol;
			reverbName =(\AutomataRMX_reverb_++id).asSymbol;
			this.declareSynths;
	...
	// We start with 0 amplitude
	declareSynths{
		Ndef(instrumentName, {
			| frames = 512, inBuffer=nil, binFilter, rate = 1, pos = 0, amp = 0, stretch = 1, shift = 0, loop = 0,
			kernel, wet = 0.0|
			var input, chain, revSignal, out;
			input = BufRd.ar(2, inBuffer, Phasor.ar(0, BufRateScale.kr(inBuffer) * rate,
				BufFrames.ir(inBuffer) * pos, BufFrames.kr(inBuffer)), loop, 2);
			chain = FFT( { LocalBuf(frames) } ! 2 , input);
			chain = PV_BinShift(chain, stretch, shift);
			chain = PV_MagMul(chain, { binFilter } ! 2);
			chain = IFFT(chain) * amp;
			revSignal = Convolution2.ar(chain, kernel, 2048);
			out = (chain * ( 1 - wet)) + (revSignal * wet);
			out
		});
		Ndef(instrumentName).mold(2);
		"Deck % ready".format(instrumentName.asString.last).postln;
	}

	runSynth { |inBuf = -1, pos = 0, rate = 1, amp = 0, stretch = 1, shift = 0, loop = 0, wet = 0.0 |
		if(inBuf != -1) {
			if(inBuf != eqBuffer.bufnum) {
				instrument = Ndef(instrumentName).set(
					\frames, frames, \inBuffer, inBuf, \binFilter, eqBuffer, \pos, pos,
					\rate, rate, \amp, amp, \stretch, stretch, \shift, shift, \loop, loop
				);
				Ndef(instrumentName).play(this.getBus); //Convolution Reverb plays
	...
	play {
		Ndef(instrumentName).play(this.getBus);
		"playing".postln;
	}

	setAndPlay { |inBuf = 1, pos = 0, rate = 1, amp = 0, stretch = 1, shift = 0, loop = 0, wet = 0.0 |
		if(isPlaying.not) {
			this.runSynth(inBuf, pos, rate, amp, stretch, shift, loop, wet);
			isPlaying = true;
			"playing".postln;
		}
	}

	fadeTo { |fadeTime = 5, newBuffer = 1, wet = 0.0|
		instrument.fadeTime = fadeTime;
		instrument.xset(\inBuffer, newBuffer);
	}
	...
	morphTo { |start, end, time, type = \lin|
		...
		if(end!=nil) {
			...
			} { "I'm playing, bitch!".postln };
		} { "Provide at least a target".postln }
	}

	// A gui to monitor changes in "current"
	morphInspector{ |winPosX = 0, winPosY = 0, masterWin|
		var winHeight = 170;
		var multiInspect;
		...
		inspectorWindow = styler.getWindow("test", Rect(winPosX, winPosY, (numBands * 4) + 38, winHeight));
		inspectorWindow.decorator = FlowLayout(inspectorWindow.bounds);
		decorator = inspectorWindow.decorator;
		styler.getSubtitleText(inspectorWindow, "Morph Inspector %".format(id+1), decorator);
		multiInspect = MultiSliderView(inspectorWindow, Rect(0, 0, (numBands * 4) + 23, winHeight * 0.75))
		.elasticMode_(1)
		.value_(zeroVals)
		.isFilled_(true)
		.indexThumbSize_(3)
		...
		.drawLines_(true)
		.fillColor_(fill)
		.strokeColor_(Color.blue.alpha_(0.5,0.1))
		.background_(Color.gray(0.2, 1));//Color.black);
		SkipJack({
			multiInspect.value = current; //Update multisliders view economically
		}, 0.05, { masterWin.isClosed }, name: "morphinspector", autostart: true);
	//	masterWin.front
	}
	...
	makeInterface {
		|inBuf = 1, pos = 0, rate = 1, amp = 1, stretch = 1, shift = 0, winPosX = 0, winPosY = 600, label = "" |
		...
		if(inBuf != nil) {
			if(inBuf != eqBuffer.bufnum) {
				...
				customFileMenu = PopUpMenu(win, (bwidth*0.6)@bheight);
